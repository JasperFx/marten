...
While projections can target specific stream or streams, it is also possible to project by event types. The following sample demonstrates this with the `CandleProjection` that extends the `MultiStreamProjection` class to build `Candle` projections from events of type `Tick`.
