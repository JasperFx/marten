...
So far, Marten has diagnostics, command logging, and unit of work life cycle tracking. In the current versions of Marten,
the built in .NET `ILogger` for your application will be the ultimate logging source for Marten activity, but this can 
still be overridden with Marten's own logging extensions. 

As of Marten 7.13, we have disabled the Npgsql tracing by default. You can reenable this -- if you really need vastly more
logging noise in your life -- with:

<!-- snippet: sample_enabling_npgsql_tracing -->
<a id='snippet-sample_enabling_npgsql_tracing'></a>
```cs
...
```
<sup><a href='https://github.com/JasperFx/marten/blob/master/src/Marten.Testing/Examples/enabling_npgsql_tracing.cs#L7-L17' title='Snippet source file'>snippet source</a> | <a href='#snippet-sample_enabling_npgsql_tracing' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->
