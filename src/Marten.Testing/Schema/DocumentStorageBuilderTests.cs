...
namespace Marten.Testing.Schema
{
    public class DocumentStorageBuilderTests
    {
        ...
        {
            DocumentStorageBuilder.Build(null, DocumentMappingFactory.For<User>()).IdType.ShouldBe(NpgsqlDbType.Uuid);
        ...
        public void do_not_blow_up_building_more_than_one()
        {
            var mappings = new[]
            {
                DocumentMappingFactory.For<User>(),
                DocumentMappingFactory.For<Company>(),
                DocumentMappingFactory.For<Issue>()
            };
