...
public class FieldMapping: IFieldMapping
{
    ...
    private readonly ConcurrentDictionary<string, IField> _fields = new();
    ...
    protected void removeIdField()
    {
        ...
        foreach (var pair in idFields) _fields.TryRemove(pair.Key, out _);
    }
    ...
    protected IField resolveField(MemberInfo[] members)
    {
        ...
        {
            if (members[members.Length - 2].GetMemberType().IsEnumerable())
            {
    ...
    private IField createFieldByFieldType(MemberInfo[] members, Type fieldType)
    {
        ...
        if (fieldType.IsEnum)
        {
            return _serializer.EnumStorage == EnumStorage.AsInteger
                ? new EnumAsIntegerField(_dataLocator, _serializer.Casing, members)
