...
namespace Marten.Linq.Model
{
    ...
    public class LinqQuery<T> : ILinqQuery
    {
        ...
        public LinqQuery(DocumentStore store, QueryModel model, IIncludeJoin[] joins, QueryStatistics stats)
        {
            ...
            _mapping = store.Tenants.Default.MappingFor(model.SourceType()).ToQueryableDocument();
        ...
        {
            var selector =
                _innerSelector = SelectorParser.ChooseSelector<T>("d.data", _store.Tenants.Default, _mapping, Model, subQuery, _store.Serializer, joins);
