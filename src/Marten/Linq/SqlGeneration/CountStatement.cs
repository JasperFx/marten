...
{
    public CountStatement(Statement parent): base(new ScalarSelectClause<T>("count(*)", parent.ExportName),
        parent.Fields)
    {
        if (parent.Mode != StatementMode.CommonTableExpression)
        {
            throw new ArgumentOutOfRangeException(nameof(parent),
                "CountStatement's parent must be a Common Table Expression statement");
        }
