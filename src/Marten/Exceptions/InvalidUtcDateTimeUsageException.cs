        ...
        public bool TryTransform(Exception original, out Exception? transformed)
        {
                ...
                ice.Message.Contains("GetTextReader method is not supported for type timestamp without time zone"))
            {
                transformed = new InvalidDateTimeUsageException(
                    $"DateTime types are no longer usable with Npgsql 6 for this use case. If using a DateTime member for optimized querying, consider using a computed index instead. Or switch this member to DateTimeOffset or use NodaTime. See https://www.npgsql.org/efcore/release-notes/6.0.html", original);
                return true;
            }
