...
{
    public StreamTypeMissingException() : base($"A stream type declaration is required when starting a new event stream because of the {nameof(EventGraph.UseMandatoryStreamTypeDeclaration)} settings. Please use one of the {nameof(IEventStoreOperations.StartStream)} overloads that take in an stream type instead and supply the stream type (usually an aggregate type)")
    {
    }
}
