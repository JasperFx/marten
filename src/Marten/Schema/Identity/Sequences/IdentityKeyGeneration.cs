...
namespace Marten.Schema.Identity.Sequences
{
    public class IdentityKeyGeneration: IIdGeneration
    {
        ...
        public void GenerateCode(GeneratedMethod method, DocumentMapping mapping)
        {
            ...
            method.Frames.Code($"if (string.{nameof(string.IsNullOrEmpty)}({{0}}.{mapping.IdMember.Name})) _setter({{0}}, \"{_mapping.Alias}\" + \"/\" + {{1}}.Sequences.SequenceFor({{2}}).NextLong());", document, Use.Type<IMartenDatabase>(), mapping.DocumentType);
