...
FROM pg_proc
         JOIN pg_namespace as ns ON pg_proc.pronamespace = ns.oid
...
WHERE NOT nspname LIKE 'pg%'
  AND i.relname like 'mt_%'
  AND pg_catalog.textin(pg_catalog.regclassout(idx.indrelid :: REGCLASS)) = :qualified_name; -- Excluding system table
...
where c.constraint_name LIKE 'mt_%'
