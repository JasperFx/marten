...
namespace Marten.Events
{
    ...
    internal class EventStream<T>: IEventStream<T>
    {
        ...
        public EventStream(EventGraph events, Guid streamId, T aggregate, CancellationToken cancellation, StreamAction stream)
        {
            _wrapper = o =>
            {
        ...
        public EventStream(EventGraph events, string streamKey, T aggregate, CancellationToken cancellation, StreamAction stream)
        {
            _wrapper = o =>
            {
                var e = events.BuildEvent(o);
