...
namespace Marten.Events
{
    internal partial class EventStore : IEventIdentityStrategy<Guid>, IEventIdentityStrategy<string>
    {
            ...
            IEventStorage selector)
        {
            ...
            {
                StreamId = id, TenantId = _tenant.TenantId
            };
            ...
            IEventStorage selector)
        {
            var statement = new EventStatement(selector)
            {
                StreamKey = id, TenantId = _tenant.TenantId
            };
    ...
    {
        Task<IEventStream<TDoc>> FetchForWriting(DocumentSessionBase session, TId id, bool forUpdate, CancellationToken cancellation = default);
        Task<IEventStream<TDoc>> FetchForWriting(DocumentSessionBase session, TId id, long expectedStartingVersion, CancellationToken cancellation = default);
    }
