...
namespace Marten.Services.Json.Transformations
{
        ...
        where TOldEvent : notnull where TEvent : notnull
    {
        ...
        /// Function takes the deserialized object of the old event type and returns the new, mapped one.</para>
    ...
    /// <summary>
    /// <para>Base implementation of <see cref="IEventUpcaster"/> transforming asynchronously JSON payload from one CLR type to the other.
    /// Upcasting is a process of transforming the old JSON schema into the new one.
    /// You can use it to handle the event schema versioning/migration.
    /// By deriving from it, you tell that for specific event type name, you'd like to get the new CLR event type.
    /// The default event type name mapping will be used based on <c>TOldEvent</c> type, unless you override <see cref="EventTypeName"/> property</para>
    /// <para>You need to provide the implementation of <see cref="UpcastAsync" /> method.
    /// It should contain the logic transforming event payload from <c>TOldEvent</c> to <c>TEvent</c>.</para>
    ...
    /// Best is to use sync transformation instead and deriving from <see cref="EventUpcaster{TOldEvent, TEvent}"/></para>
    /// <para>Use <c><see cref="Marten.Events.IEventStoreOptions.Upcast">store.options.Upcast</see></c> method to register upcaster implementation
    /// and tell Marten that you'd like to use it.</para>
    /// </summary>
    /// <example>
        ...
        where TOldEvent : notnull where TEvent : notnull
    {
        ...
        /// <summary>
        /// <para>Method defines the asynchronous event JSON payload transformation. It "upcasts" one event schema into another.
        /// You can use it to handle the event schema versioning/migration.</para>
        /// <para>By defining it, you tell that instead of the old CLR type, for the specific event type name,
        /// you'd like to get the new CLR event type.
        /// Function takes the deserialized object of the old event type and returns the new, mapped one.</para>
