...
/// <summary>
/// Base class for single-stream aggregate projections that persist the aggregate
    ...
    where TDoc : class where TId : notnull where TDbContext : DbContext
{
    ...
    /// </summary>
    [JasperFxIgnore]
    public virtual ValueTask<(TDoc?, ActionType)> ApplyEventsAsync(
    ...
    /// <see cref="SingleStreamProjection{TDoc,TId}"/> validation which assumes Marten document storage.
    /// </summary>
