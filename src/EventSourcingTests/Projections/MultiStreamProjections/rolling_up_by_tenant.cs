...
public class rolling_up_by_tenant : OneOffConfigurationsContext
{
    ...
    [Fact]
    public async Task track_totals_by_tenant_id_using_strong_typed_id()
    {
        StoreOptions(opts =>
        {
            opts.Events.TenancyStyle = TenancyStyle.Conjoined;
        ...
        (await theSession.LoadAsync<Rollup2>(new TenantId("three"))).ACount.ShouldBe(4);
    }
}
...
public class Rollup2Projection: MultiStreamProjection<Rollup2, TenantId>
{
    ...
    public void Apply(Rollup2 state, AEvent e) => state.ACount++;
    public void Apply(Rollup2 state, BEvent e) => state.BCount++;
}

public class Rollup2
{
    [Identity]
