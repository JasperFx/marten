...
public sealed class CompiledQuerySourceGenerator : IIncrementalGenerator
{
        ...
        string caseBodies, string includeReaderCtors)
    {
        ...
        // The three *FullName constants below NAME a type; they are not code positions. `global::`
        // is C# source syntax for resolving an alias ambiguity, not part of any type's name, so
        // emitting "global::Sample.FindByName" as a runtime string would hand the wrong value to
        // anything that later looks the type up or logs it. Everything above this line is a real
        // emit position and keeps the prefixed form.
        //
        // Replaces every occurrence rather than trimming a leading one: a closed generic renders as
        // global::System.Collections.Generic.List<global::Sample.Doc>, and the inner prefix is just
        // as wrong in a name.
        var describedQuery = describe(fullName);
        var describedDocType = describe(docType);
        var describedOutputType = describe(outputType);

        return $$"""
            // <auto-generated/>
            // Marten.SourceGenerator — #4405 PoC iteration 3.
            // Discovered compiled query: {{describedQuery}}
            //   Doc type:    {{describedDocType}}
            //   Output type: {{describedOutputType}}
            //
            ...
            {
                public const string DiscoveredQueryFullName = "{{describedQuery}}";
                public const string DocTypeFullName = "{{describedDocType}}";
                public const string OutputTypeFullName = "{{describedOutputType}}";
    ...
    /// <summary>
    /// Turns an <see cref="Emit"/>ed name back into the plain type name, for the positions that
    /// describe a type rather than compile as one — the <c>*FullName</c> string constants and the
    /// header comment. See the note at the call site in <c>EmitSource</c>.
    /// </summary>
    private static string describe(string emitted)
        => emitted.Replace("global::", string.Empty);
