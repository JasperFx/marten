...
* **Next milestone (iteration 3):** runtime first-call LINQ-parse plumbing. `QueryCompiler.BuildQueryPlan` already runs codegen-free; bypass `CompiledQuerySourceBuilder.AssembleTypes` and use the source-gen scaffold to hold the SQL + parameter binder. Memoize forever. Shape: a `[ModuleInitializer]` emitted alongside each handler class registers it with a runtime registry keyed by query `Type`. **Opt-in is implicit** — the registry is populated by the generator-emitted module initializers; no `StoreOptions` flag. **No fallback at ship**: a registry miss throws `InvalidOperationException` directing the user to add a `Marten.SourceGenerator` package reference + `[assembly: JasperFxAssembly]`. During the PoC the miss branch falls through to the existing `JasperFx.RuntimeCompiler` codegen path so the scattered tests keep working; that bridge is removed once iteration 4 lands green.
